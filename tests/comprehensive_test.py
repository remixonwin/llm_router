...
def test_endpoint(name, path, payload, expected_status=200):
    ...
    if resp.status_code == 200:
        ...
        print(f"Routed via: {meta.get('provider')} / {meta.get('model')} (Strategy: {meta.get('strategy')})")
...
if __name__ == "__main__":
    # 1. Text-only Chat
    test_endpoint("Text Chat", "/v1/chat/completions", {
        "messages": [{"role": "user", "content": "What is the capital of France?"}],
        "routing": {"strategy": "cost_optimized"}
    })

    # 2. Vision (Base64 URL) via /v1/vision
    test_endpoint("Vision Base64 URL", "/v1/vision", {
    ...
    test_endpoint("Vision Base64 Direct", "/v1/vision", {
    ...
    test_endpoint("Mixed Chat (URL)", "/v1/chat/completions", {
        "messages": [{
            "role": "user",
            "content": [
                {"type": "text", "text": "Describe this image."},
                {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{RED_DOT_BASE64}"}}
            ]
        }],
    ...
    test_endpoint("Mixed Chat (Base64)", "/v1/chat/completions", {
        "messages": [{
            "role": "user",
            "content": [
