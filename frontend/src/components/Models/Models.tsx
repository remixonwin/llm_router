...
export function Models() {
  ...
  const filteredModels = useMemo(() => {
    ...
    return data.data.filter((model: Model) => {
      const matchesSearch = !search || model.id.toLowerCase().includes(search.toLowerCase());
      const matchesProvider = !providerFilter || model.owned_by === providerFilter;
              ...
              {filteredModels.map((model: Model) => (
                <tr key={model.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <span className="font-mono text-sm text-gray-900">{model.id}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="capitalize text-sm text-gray-600">{model.owned_by}</span>
                  </td>
